...
- [ ] **Always-online assumptions to harden later** (flagged, not fixed in the PWA PR):
  - Rappel **chat send + voice TTS** use raw `fetch` (not the `api.js` wrapper), so they bypass the offline guard — offline they fail with a raw error, no friendly message.
  - Core navigation GETs aren't cached (`/courses`, `/me/stats`, `/me/progress`, `/me/activity`, `/units/:id`, `/units/:id/documents`, `/preferences`) — offline, Home/Courses/Progress/Unit pages can't render, so you may not be able to *navigate* to the cached quizzes/flashcards. Consider adding these to runtime cache.
  - Pages that `return null` / show infinite "Loading…" on a failed GET (CoursePage, UnitPage, DeckPage, StudyGuidePage, QuizPage) hang offline with no "couldn't load — you're offline" state.
