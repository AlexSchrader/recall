...
- [~] **Real PWA + offline review + one-tap install** — `vite-plugin-pwa` (generateSW): manifest generated (192/512 + maskable, `#2563EB` theme, `#f5f5f7` bg, standalone, scope `/`), app-shell precache, runtime caching — `StaleWhileRevalidate` for GET `/api/{quizzes,flashcards,chat/threads}`, `CacheFirst` for images, `NetworkOnly` for `/api/billing`, navigation denylist on `/api`. SW registered via `src/pwa.js` with a **prompt-to-refresh** banner (`UpdatePrompt`, no mid-session auto-reload). `main.jsx` no longer unregisters SWs, so `beforeinstallprompt` now fires → install guide's one-tap button works on Chromium (iOS stays manual). `<OfflineIndicator />` pill in header; `api.js` blocks mutations offline with a friendly message (GETs still hit the SW cache). Build verified (sw.js + manifest + routes inspected); **STARTED 2026-06-27 — flip to [x] only after real-device testing (see below).** (CC)
- [!] **BLOCKED ON: real-device verification (Alex).** SWs can't be verified in local dev (plugin `devOptions` disabled) and I can't drive real browsers — needs a deploy + manual pass: (1) desktop Chrome: install via address-bar icon / install-guide button; (2) Android Chrome: one-tap install fires; (3) iOS Safari: Share → Add to Home Screen still works; (4) offline (DevTools → Offline or airplane mode): a previously-opened quiz/flashcard/chat still loads; (5) ship a change → "new version ready" banner appears, Refresh applies it. Verify manifest + SW register cleanly in the Application tab.
