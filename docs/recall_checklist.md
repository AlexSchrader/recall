...
- [ ] **CI blocked by pre-existing repo-wide lint debt** — `npm run lint` fails on ~36 `eslint` errors (unused vars, `react/no-unescaped-entities`, useless escapes, empty blocks) spread across client + server, none from the test harness. CI runs lint *before* test, so the new suite never runs on a PR until this is cleared. Needs a dedicated mechanical lint-cleanup pass (verify each fix is zero-behavior, e.g. don't alter prompt strings). Flagged 2026-06-24. (CC)
...
- **CI's `pull_request` trigger only fires on PRs to `dev`, so work cherry-picked/merged straight to `main` never runs CI.** Combined with a repo-wide **pre-existing lint debt** (~36 `eslint` errors across client + server as of 2026-06-24, unrelated to any one change), `npm run lint` fails — and since CI runs lint *before* test, the test job never reaches the (now substantial) suite. The integration harness is correctly wired into `npm test`, but **CI won't actually run it until the lint debt is cleared.** See the open Phase 6 item.
