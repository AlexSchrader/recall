...
- [x] Per-topic mastery **trend arrows** (improving/declining) on Progress — `GET /me/progress` now attaches a `trend` (7-day delta in percentage points) per topic: `getMasteryBaselines()` finds each topic's mastery as of the cutoff (newest history row at/before 7 days ago; falls back to its earliest score if first seen inside the window), and the route returns `current − baseline`. UI shows a ▲green/▼red `±N%` next to the mastery pill, hidden when flat or when there's no history yet (`trend: null`). 3 harness tests (cutoff delta, new-topic fallback, null-on-no-history) — DONE 2026-06-27 (CC)

- [x] **Guided spotlight tour** — first-run walkthrough that dims the screen and rings the real UI (Courses/Rappel/Games/Progress nav, "+ New course", Settings) with an animated primary-color ring + coachmark per step (Next/Back/Skip, step counter). Targets found via `data-tour` attrs; `Tour.jsx` re-measures on scroll/resize. Onboarding now ends by launching it (`/?tour=1`); replayable from Settings → "Getting started → Replay app tour" (`tourSeen` localStorage flag) — DONE 2026-06-27 (CC)

**PWA / offline (2026-06-27)**
- [x] **Real PWA + offline review + one-tap install** — `vite-plugin-pwa` (generateSW): manifest generated (192/512 + maskable, `#2563EB` theme, `#f5f5f7` bg, standalone, scope `/`), app-shell precache, runtime caching — `StaleWhileRevalidate` for GET `/api/{quizzes,flashcards,chat/threads}`, `CacheFirst` for images, `NetworkOnly` for `/api/billing`, navigation denylist on `/api`. SW registered via `src/pwa.js` with a **prompt-to-refresh** banner (`UpdatePrompt`, no mid-session auto-reload). `main.jsx` no longer unregisters SWs, so `beforeinstallprompt` now fires → install guide's one-tap button works on Chromium (iOS stays manual). `<OfflineIndicator />` pill in header; `api.js` blocks mutations offline with a friendly message (GETs still hit the SW cache). **VERIFIED on real devices 2026-06-27 (Alex) — install, offline review, and update banner all pass.** (CC)
- [x] **Real-device verification (Alex)** — install (desktop Chrome / Android Chrome / iOS Safari), offline review of cached quiz/flashcard/chat, and the update banner all confirmed working on real hardware 2026-06-27. No round-2 offline findings to fix.
- [x] **Offline hardening (round 1)** — DONE 2026-06-27 (CC):
  - Rappel **chat send** now shows a friendly "you're offline — past chats still readable" message instead of a raw error; **voice TTS** stays quiet offline.
  - **Core navigation GETs cached** (`recall-nav-api` SWR: `/api/{courses,units,me,preferences}`) so Home/Courses/Progress/Unit pages render offline — you can now actually *navigate to* cached quizzes/flashcards/chats.
- [ ] **Always-online assumptions still to harden:**
  - Pages that `return null` / show infinite "Loading…" on a failed GET (CoursePage, UnitPage, DeckPage, StudyGuidePage, QuizPage) hang offline if that data was never cached — no "couldn't load — you're offline" state.
  - Document upload + voice unlock/preview assume online; no graceful offline messaging.
...
- [x] Feedback route integration test — auth, validation, and email-fired coverage; the route was previously untested — DONE 2026-06-28 (CC). (A short-lived "Open Admin → Feedback" email button was added then removed at Alex's request — feedback already lands in both the inbox and the admin tab, so the link was redundant.)
...
### Engagement Branch 3 — Polish *(2026-07-11, `feat/branch-3-polish`)*
- [x] **Keyboard shortcuts for the drill games** — shared `useOptionKeys` hook: press 1–4 (up to 1–9) to pick the matching option in Speed Round, Time Attack, Streak, Survival, and Boss Battle. Inert between questions / after answering / in text inputs. Desktop-only `.kbd-hint` chip under the options (hidden on touch) for discoverability — DONE 2026-07-11 (CC)
- [x] **Completion celebration** — Confetti fires on the quiz result screen for a great score (≥ 90%). Reuses the shared `Confetti` component — DONE 2026-07-11 (CC)
...
- [x] **[SEV-2] Resend client boot-crash removed** — `email.js` built `new Resend(process.env.RESEND_API_KEY)` at *import* time, and the Resend constructor **throws** `"Missing API key"` when the key is unset → the whole server would crash on boot (email.js is imported by the auth + feedback routes). Now lazily constructed in `resendClient()` on first send, so a missing key only breaks the email path (already `.catch`-ed) instead of the app. Verified: `email.js` imports clean with `RESEND_API_KEY` unset. Found by the `recall-code-auditor` gate on PR #12 — DONE 2026-07-12 (CC)
...
- [ ] **[SEV-2 · confirmed] Server hard-crashes on boot when `RESEND_API_KEY` is unset.** `services/email.js:3` calls `new Resend(process.env.RESEND_API_KEY)` at module-import time, and the Resend constructor throws `"Missing API key"` when the var is empty/absent. Because `email.js` is imported by the route graph at startup, this takes down the **entire** server (auth, quizzes, games — everything), not just email. Contrast the fail-closed pattern already used elsewhere: the Anthropic client boots fine with a dummy key and fails lazily at call-time, and ElevenLabs voice routes 503 when unconfigured. Resend is the lone service that crashes at import. **Prod impact today: none** (Railway has the key set), but it's a latent footgun — rotate/empty that env var, or a new dev/CI without it, and the service dies on start rather than merely losing email. **Repro:** `RESEND_API_KEY= npm run dev` → server crash-loops with the Resend error; app never comes up. **Fix (Dev, small + safe):** lazily construct the Resend client inside `sendFeedback`/`sendPasswordReset` (or guard on a missing key and no-op/log), matching the lazy pattern the other paid services use — with a regression test that imports the app with `RESEND_API_KEY` unset and asserts it still boots. Diagnosed 2026-07-07 during a QA run-through.
- [~] **[SEV-1 · confirmed] Feedback emails never deliver — Resend returns 403 Forbidden on every `/emails` POST.** Feedback is saved to SQLite and the admin tab, but `services/email.js` sends from `onboarding@resend.dev` with no verified domain, so Resend refuses delivery to `recallstudyapp.support@gmail.com` (testing mode only allows sending to the Resend account's own signup address). **Root cause of the silence found:** the Resend SDK *resolves* with `{ data, error }` instead of throwing on an API error, so the route's `.catch` never fired. **App-hardening half DONE 2026-07-11 (CC, Branch 5):** new `unwrapResend()` helper throws when Resend returns an `error`, so both `sendFeedback` + `sendPasswordReset` now propagate the real reason and the feedback route logs `[feedback] email failed: Resend send failed: …` instead of masquerading as success. 4 unit tests. **Still open — account/config (Alex):** verify a domain in Resend (then Dev sets a real `FROM_EMAIL`), OR re-register the Resend account under the destination inbox, so 403s stop entirely. Diagnosed 2026-06-28.
- [x] **[SEV-2 · gate-surfaced] Resend client constructed at import → boot-crash if `RESEND_API_KEY` unset.** Surfaced by `recall-code-auditor` during the first live fleet run on PR #12. The Resend constructor throws `"Missing API key"`; building it at module scope meant an unset key would crash the server on boot (email.js is imported by auth + feedback routes). **Fixed** in Branch 5: lazy `resendClient()` builds on first send, containing any missing-key failure to the email path. Verified `email.js` imports clean with the key unset — DONE 2026-07-12 (CC).

**Fleet QA sweep of the merged engagement push (2026-07-12, `fix/qa-sweep-followups`):**
- [x] **[SEV-2 · gate-surfaced] "Redo my mistakes" dead-ended on true/false.** `RedoPage` treated `true_false` as interactive and built buttons from `options_json`, but the generator stores `options_json: NULL` for non-MCQ types → zero buttons, no reveal, no way to advance; any redo queue containing a missed T/F was uncompletable. **Fixed:** `INTERACTIVE = new Set(['mcq'])` only — every other type uses the flip-to-reveal path — DONE 2026-07-12 (CC).
- [x] **[hygiene · gate-surfaced] SQLite artifacts were tracked in git.** `server/data/recall.db-shm` + `recall.db-wal` were committed (the `.gitignore` pattern `*.db` didn't match `.db-wal`/`.db-shm`). **Fixed:** `git rm --cached` + broadened ignore to `server/data/*.db-wal|-shm|-journal` — DONE 2026-07-12 (CC).
- [x] **[design · truth-agent] Daily Mix inflated the Exam-Readiness signal it shares.** Daily Mix pulled MCQs via `ORDER BY RANDOM()` (no due/weak gate) and fed `/games/results` → SM-2 → `topic_mastery.mastery`, the same field Exam Readiness + trend arrows read. Grinding already-mastered topics off-schedule drifted readiness upward for the most engaged user. **Fixed (option a):** added a weak-first `listGameWeak` query (`LEFT JOIN topic_mastery … ORDER BY MIN(mastery) ASC, RANDOM()`), exposed as `GET /games/questions?weak=1`; Daily Mix now requests `weak=1` so it drills the weakest topics (legit learning that *should* move readiness) instead of random known ones. 2 regression tests (weak-first ordering; weak mode still user-scoped) — DONE 2026-07-12 (CC).
